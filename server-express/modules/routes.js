...
function registrarRotas(server, contexto) {
    ...
    return _
        ...
        .reduce((lista, pasta) => {
            try {
                let apiConfig = require(path.join(contexto.DIRETORIO_SERVER, systemConfig.dir.apis, pasta,
                    systemConfig.package));
                if (_.get(apiConfig, 'type') === systemConfig.type.api) {
                    ...
                    lista.push({
                        ...
                        controller: path.join(contexto.DIRETORIO_SERVER, systemConfig.dir.apis, pasta, _.get(
                            apiConfig, systemConfig.default.controller, '')),
                        functions: _
                            .chain(systemConfig.handlers)
                            .filter(funcao => _.get(apiConfig, funcao.name, false) === true || funcao.required ===
                                true)
        ...
        .filter(api => {
            return (_.isString(api.route) && _.size(api.route) > 0) &&
                ...
                (_.isString(api.controller) && _.size(api.controller) && (new RegExp(systemConfig.regex.verify_controller,
                    'g')).test(api.controller)) &&
